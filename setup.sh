...
function setup_mysql_database() {
  ...
  if [ "$(az mysql flexible-server show --name ${MYSQL_INSTANCE_NAME} --resource-group ${RESOURCE_GROUP} --query "name == '${MYSQL_INSTANCE_NAME}'" || echo "empty")" == "true" ]; then
    ...
    az mysql flexible-server create --location "${LOCATION}" --resource-group "${RESOURCE_GROUP}" \
      --name "${MYSQL_INSTANCE_NAME}" --admin-user "${MYSQL_GITPOD_USERNAME}" --admin-password "${MYSQL_GITPOD_PASSWORD}" \
      --sku-name Standard_B2s --tier Burstable --public-access 0.0.0.0 --storage-size 20 \
      --version 5.7 --storage-auto-grow Enabled
  fi
  ...
  if [ "$(az mysql flexible-server db show --database-name ${DB_NAME} --resource-group ${RESOURCE_GROUP} --server-name ${MYSQL_INSTANCE_NAME} --query "name == '${DB_NAME}'" || echo "empty")" == "true" ]; then
